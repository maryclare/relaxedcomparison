...
## -----------------------------------------------------------------------------
load("./first_value_log_equal_spacing.rda")
rm(list = setdiff(ls(), "grid"))
lambda_start = grid$lambda[7] # nzs = 7
lambda_end = grid$lambda[8] # nzs = 10
...
for (r in 1:nrep) {
  ...
  # lambda_seq_manual_25 <- seq(lambda_max_standardized_25, lambda_min_25, length.out = nlam)
...
# save(list=ls(),file="refined_grids_independent.rda")
...
ggplot(dat25, aes(x = log(lambda), y = nzs)) +
  ...
  theme_minimal()
# ggsave("refined_grids.pdf", height=6, width=8, device="pdf")
unique(dat25$nzs)
# [1]  0  1  2  3  4  6  7 10 13 14 17 19 20 21 22 24 25 27 28 29 30
library(dplyr)
grid = dat25 %>%
  group_by(nzs) %>%
  summarise(lambda = first(lambda), .groups = "drop")
# Fit an exponential model
exp_model <- nls(lambda ~ a * exp(-b * nzs) + c, data = grid, 
                 start = list(a = 2, b = 0.1, c = 0))
# summary(exp_model)
# Generate predictions
grid$fit <- predict(exp_model)

# Plot with ggplot2
ggplot(grid, aes(x = nzs, y = lambda)) +
  geom_point(color = "blue") +  # Data points
  geom_line(aes(y = fit), color = "red", linetype = "dashed") +  # Fitted curve
  labs(title = "Nonzeros vs Lambda with Exponential Fit", x = "nonzeros", y = "lambda") +
  theme_minimal()

# ggsave("first_value_2.pdf", height=6, width=8, device="pdf")
# save(list=ls(),file="first_value_2.rda")
...
ggplot(combined_df, aes(x = log(lambda), y = nzs, color = group)) +
  ...
  theme_minimal()
# ggsave("refined_grids_independent.pdf", height=6, width=8, device="pdf")
