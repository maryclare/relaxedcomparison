...
save(list=ls(),file="sim.df_new_q_only.rda")
...
ggplot(dat25, aes(x = log(lambda), y = nzs)) +
  ...
  theme_minimal()
ggsave("lambda_nzs_25_epsilon_5.pdf", height=6, width=8, device="pdf")
...
ggplot(dat50, aes(x = log(lambda), y = nzs)) +
  ...
  theme_minimal()
ggsave("lambda_nzs_50_epsilon_5.pdf", height=6, width=8, device="pdf")
...
ggplot(combined_df, aes(x = log(lambda), y = nzs, color = group)) +
  ...
  theme_minimal()
ggsave("lambda_nzs_all.pdf", height=6, width=8, device="pdf")
